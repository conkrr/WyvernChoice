	...
	public List<Disapproval> generate(ResultSet resultSet) throws Exception {
        ...
        try {
			while(resultSet.next()) {
				...
				if (approveStatus == -1) {
					approvals.add(new Disapproval(alternativeId, userId, timestamp, userId, choiceID));
				}
